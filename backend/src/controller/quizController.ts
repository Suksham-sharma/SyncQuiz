...
export const getAllQuiz = async (req: Request, res: Response) => {
  ...
  try {
    const quiz = await db
      .select()
      .from(quizTable)
      .where(eq(quizTable.createdBy, userId))
      .execute();
...
export const getQuizById = async (req: Request, res: Response) => {
  ...
  const userId = req.user.id;
  try {
    const quizWithDetails = await db.query.quizTable.findFirst({
...
export const deleteQuiz = async (req: Request, res: Response) => {
  ...
  try {
    const quiz = await db
      ...
      .from(quizTable)
      .where(
        and(eq(quizTable.id, Number(quizId)), eq(quizTable.createdBy, userId))
      )
      .execute();
    ...
    const deleteQuizId = await db
      .delete(quizTable)
      .where(eq(quizTable.id, Number(quizId)))
      .returning({ deletedId: quizTable.id });
...
export const createQuiz = async (req: Request, res: Response) => {
  ...
  try {
    if (
      ...
      res.status(400).json({
        error: "validation_error",
        message: "Error in heading/description",
      });
      return;
    }
...
export const addQuizQuestions = async (req: Request, res: Response) => {
  ...
  try {
      ...
      for (const questionData of questions) {
        ...
        if (correctOptionId) {
          await tx.insert(answerTable).values({
            ...
            answerDescription:
              questionData.answers?.[0]?.answerDescription || "Correct Answer",
          });
        }
      }
    });
    ...
    res
      .status(201)
      .json({ message: "Questions, options, and answers added successfully" });
    return;
  } catch (error) {
    console.error(error);
